...
const getButtonStyle = (style: string | undefined, primaryColor: string, surfaceColor: string, inputBorder: string) => {
  switch (style) {
    case 'destructive':
      return { backgroundColor: '#dc3545' };
    case 'cancel':
      return { backgroundColor: surfaceColor, borderWidth: 1, borderColor: inputBorder };
    case 'save':
        ...
        <View style={[styles.modal, { backgroundColor: colors.surface, borderColor: colors.inputBorder }]}>
          ...
          <View style={styles.buttonContainer}>
            {buttons.map((button, index) => (
              <TouchableOpacity
                key={index}
                style={[styles.button, getButtonStyle(button.style, colors.primary, colors.surface, colors.inputBorder)]}
                onPress={() => {
                  button.onPress();
                  onClose();
                }}
